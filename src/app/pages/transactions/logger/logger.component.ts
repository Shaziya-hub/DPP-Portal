...
export class LoggerComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.transId = this.transactionId;
    ...
    if ((this.transId?.columnName == "transId" || this.instanceId?.columnName == "ePayInstanceId") && (this.transactionsPageInfo?.pageName == 'payments' || this.transactionsPageInfo?.pageName == 'refunds' || this.transactionsPageInfo?.pageName == 'einvoices' || this.transactionsPageInfo.pageName == keyWords.payouts)) {
      let filters = {
        SearchFilters: {
          transId: this.transId?.key,
          instanceId: this.instanceId?.key
        }
      }
      this.loggerfilters = filters
    } else if ((this.transactionsPageInfo?.pageName == 'bills' || this.transactionsPageInfo?.pageName == 'payments' || this.transactionsPageInfo?.pageName == 'batches' || this.transactionsPageInfo?.pageName == "payrolls") && (this.transId?.columnName == "batchId" || this.instanceId?.columnName == "ePayInstanceId")) {
      let filters = {
        SearchFilters: {
          batchId: this.transId?.key,
          instanceId: this.instanceId?.key
        }
      }
      this.loggerfilters = filters
  ...
  navigation(val) {
    ...
    val = this.transactionId
    this.transactionIdEvent.emit(val);
    this.LoggerEvent.emit('backFromLogger');
  ...
  setGridColumns(dataSource) {
    ...
    this.selectableColumns && this.selectableColumns.map((o: any) => {
      if (o.flag == 'true') {
        if (o['id'].toLowerCase().indexOf('requestdata') > -1) {
          this.columns.push({ dataField: o['id'], caption: o['name'], allowSorting: false, adaptive: true, cssClass: "requestdata-css", cellTemplate: "requestCellTemplate" });
        } else if (o['id'].toLowerCase().indexOf('replydata') > -1) {
          this.columns.push({ dataField: o['id'], caption: o['name'], allowSorting: false, adaptive: true, cssClass: "replydata-css", cellTemplate: "replyCellTemplate" });
        } else {
